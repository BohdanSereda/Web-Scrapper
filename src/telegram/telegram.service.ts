...
export class TelegramService {
    ...
    async showCities(ctx: Context): Promise<Message.TextMessage> {
        ...
        const reply = cities
            .map((city: string, index: number) => `${index + 1}. ${city}`)
            .join('\n');
        ctx.session.type = '';
    ...
    async showRestaurants(
        ...
        const rawRestaurants = await DataBaseHelper.getBusinessesByCity(
            message,
    ...
    async showWorkingHours(
        ctx: Context,
        message: string,
    ): Promise<Message.TextMessage> {
        const restaurant = await DataBaseHelper.getRestaurant(
            message,
            this.businessRepository,
        );
        if (!restaurant) {
        ...
        const reply = restaurant.workingHours
    ...
    async reserveDay(
        ...
        const isValid = await TelegramBotValidator.dateValidation(
            date,
            ctx,
            workingHours,
        );
    ...
    async reserveWorkingHours(
        ctx: Context,
        message: string,
    ): Promise<Message.TextMessage> {
        const workingHours = ctx.session.restaurant.workingHours;
        const time = moment(message, 'hh:mm A');
        const isValid = await TelegramBotValidator.timeValidation(
            time,
            ctx,
            workingHours,
        );
