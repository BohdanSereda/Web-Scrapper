...
export class EmailHelper {
    async sendReservationEmail(mailerService: MailerService, updateReservation: CreateReservationDto): Promise<void> {
        const templates = {
    ...
    async sendScrapingResultEmail(mailerService: MailerService, emailData: EmailScrapingDataDto): Promise<void>{
        const templates = {
        ...
        await this.sendEmail(mailerService, emailData.from, emailData.subject, emailData.email, templates[emailData.status])
    }

    async sendEmail(mailerService: MailerService, from: string, subject: string, to: string, text: string): Promise<void>{
