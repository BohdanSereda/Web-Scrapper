...
export class ScraperService {
    ...
    async scrapeBusinessData(
        scrapeBusinessDto: ScrapeBusinessDto,
    ): Promise<string> {
        ...
        try {
            const existBusinesses = await DataBaseHelper.getBusinessesByCity(
                city,
            ...
            if (existBusinesses.length) {
                ...
                await emailHelper.sendScrapingResultEmail(
                    this.mailerService,
                    emailData,
                );
            ...
            for (let i = 0; i < 50; i += 10) {
                ...
                const link =
                    ...
                    '&start=' +
                    i;
                const businessListPageHtml = await pageScraperHelper.scrapePage(
                    link,
                );
                await TimerHelper.timer(15000);
                businessesLinks.push(
                    ...this.getBusinessLinks(businessListPageHtml),
                );
            ...
            for (const businessesLink of businessesLinks) {
                ...
                const businessLink =
                    ...
                    businessesLink +
                    '&sort_by=rating_asc';
                const businessPageHtml = await pageScraperHelper.scrapePage(
                    businessLink,
                );
                ...
                const business: GetBusinessDto =
                    await informationScraperHelper.extractBusinessPageInformation(
                        businessLink,
                        city,
                    );
                if (!business) {
                ...
                const savedBusiness = await DataBaseHelper.createUniqueBusiness(
                    business,
                    this.businessRepository,
                );
            ...
            console.log(
                `scraped ${businessesData.length} businesses, city: ${city}`,
            );
    ...
    async getBusinesses(city: string): Promise<Business[]> {
        const businesses = await DataBaseHelper.getBusinesses(
            city,
            this.businessRepository,
        );
