...
export class BusinessEventValidator {
    static dayCoincidenceValidation(
        ...
        return (
            start.day() === existingEventStart.day() ||
    ...
    static datePeriodValidation(
        ...
        return (
            normalizedStart.isBetween(
                normalizedExistingStart,
                normalizedExistingEnd,
            ) ||
            normalizedEnd.isBetween(
                normalizedExistingStart,
                normalizedExistingEnd,
            ) ||
    ...
    static dateIntervalValidation(data: DateIntervalData): string | false {
        ...
        const isInInterval = BusinessEventValidator.datePeriodValidation(
            normalizedStart,
            normalizedEnd,
            normalizedExistingEnd,
            normalizedExistingStart,
    ...
    static async dateValidation(
        ...
        let start = moment(
            createBusinessEventDto.event_start,
            'DD MM YYYY, hh:mm',
        );
        let end = moment(createBusinessEventDto.event_end, 'DD MM YYYY, hh:mm');
        const today = moment().startOf('day');
        const events = await DataBaseHelper.getBusinessEvents(
                ...
                case 'weekly': {
                    const unFilteredEvents = events.some((event) => {
                        start = moment(
                            ...
                            createBusinessEventDto.event_end,
                            'DD MM YYYY, hh:mm',
                        );
                        ...
                        const existingEventEnd = moment(
                            event.event_end,
                            'DD-MM-YYYY hh:mm',
                        );
                        const coincided =
                            BusinessEventValidator.dayCoincidenceValidation(
                                start,
                                end,
                                existingEventStart,
                                existingEventEnd,
                            );
                        const normalized = BusinessEventValidator.normalization(
                            start,
                            end,
                            existingEventStart,
                            existingEventEnd,
                        ...
                        const data = {
                            event,
                            normalized,
                            context: createBusinessEventDto.frequency,
                            coincided,
                        };

                        const validated =
                            BusinessEventValidator.dateIntervalValidation(data);
