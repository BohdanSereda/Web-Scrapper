...
export class InformationScraperHelper {
    ...
    scrapeName = (): string => {
        return this.$('div[data-testid="photoHeader"]')
            .find('h1')
            .text()
            .trim();
    };
    ...
    scrapeAmenities = (): string[] => {
        return this.$('.arrange__09f24__LDfbs')
    ...
    scrapeRating = (): number => {
        ...
        const startIndx =
            headerHtml.indexOf('aria-label="') + 'aria-label="'.length;
    ...
    scrapePhone = (): string => {
        ...
        this.$('p.css-1p9ibgf')
            .filter((i, el: Element) =>
                /[1-9]{1}[0-9]{3,14}$/.test(this.$(el).text()),
            )
            .each((i, el: Element) => {
    ...
    scrapeHighestRatedReview = async (
        businessLink: string,
    ): Promise<string> => {
        const pageScraperHelper = new PageScraperHelper();
        const businessPageLinkSortedByDesc =
            businessLink.substring(0, businessLink.length - 4) + '&desc';
        TimerHelper.timer(4000);
        const businessPageHtmlSortedByDesc = await pageScraperHelper.scrapePage(
            businessPageLinkSortedByDesc,
        );
        const $$ = load(businessPageHtmlSortedByDesc);
        return $$('.comment__09f24__gu0rG').find('span').first().text();
    };

    extractBusinessPageInformation = async (
        businessLink: string,
        city: string,
    ) => {
        ...
        const highest_rated_review: string =
            await this.scrapeHighestRatedReview(businessLink);
        return {
            name,
            description,
            images,
